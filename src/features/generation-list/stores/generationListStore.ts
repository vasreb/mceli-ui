...
import type { BaseQueryItem } from '../../../api/types';

export type GenerationStatus = 'pending' | 'running' | 'done' | 'failed' | 'in_progress';

export interface Generation {
  id: string;
  status: GenerationStatus;
  createdAt: string;
  baseQueries: BaseQueryItem[];
  metrics: {
    baseQueries: number;
    rootQueries: number;
  ...
  async loadGenerations() {
    ...
    try {
      ...
      this.generations = response.items.map((item) => {
        const baseSnapshots = item.stats.baseQueriesSerpSnapshotsCount || 0;
        const rootsSnapshots = item.stats.rootsSerpSnapshotsCount || 0;
        const variantsSnapshots = item.stats.variantsSnapshotsCount || 0;
        const totalSnapshots = baseSnapshots + rootsSnapshots + variantsSnapshots;
        ...
        return {
          ...
          metrics: {
            baseQueries: item.stats.baseQueriesCount || item.baseQueries?.length || 0,
            rootQueries: item.stats.rootsCount || 0,
            variantQueries: item.stats.variantsCount || 0,
            clusterQueries: item.stats.clustersCount || 0,
          },
          snapshots: totalSnapshots,
          stats: {
            baseQueriesSerpSnapshotsCount: baseSnapshots,
            rootsSerpSnapshotsCount: rootsSnapshots,
            variantsSnapshotsCount: variantsSnapshots,
          },
